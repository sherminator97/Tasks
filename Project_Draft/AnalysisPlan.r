...
my_data2 <-read.csv("Anolis_morph_data.csv")
...
#make tree
install.packages("GenBank")
library(phytools)
tree <- force.ultrametric(read.tree("morph_data.csv"))

#chisquare test for forelimb length
morph.data <- data.frame(my_data$Species, my_data$SVL)
morph.data = table(my_data$Species, my_data$FL)
print(morph.data)
print(chisq.test(morph.data))

#plot for species vs snout-vent length
...
#chisquare test for Anolis snout-vent length
Anolismorph.data <- data.frame(my_data2$Species, my_data2$SVL)
Anolismorph.data = table(my_data2$Species, my_data2$SVL)
print(Anolismorph.data)
print(chisq.test(Anolismorph.data))

#plot for species vs eye to limb length
pdf("Anolis Species vs. snout vent-length.pdf", height=8, width=8)
par(las=1, mar=c(5, 5, 1, 1), mgp=c(2, 0.5, 0), tck=-0.01)
plot(my_data2$Species, my_data2$SVL, xlab="Species", ylab="snout-vent length")
dev.off()
